...
**Engagement ID:** `eng-20260624111036-syfe-fullpower-healed-2026`  
**Generated At:** `2026-06-24 11:22:53 UTC`  
...
| **Raw URLs Discovered** | 265 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 53 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 53 | Graph Persistence Layer |
| **Reportable Endpoints** | 53 | Graph Memory Query |
...
| **Expansion Ratio** | **84x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (53) + \text{Parameters} (0) + \text{API Routes} (0) + \text{JS Bundles} (31)}{\text{Input Targets} (1)} = 84x$$
...
|---|---|---|
| **Anonymous-only Routes** | 53 | Accessible without session credentials |
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (84x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

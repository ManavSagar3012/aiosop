...
**Generated At:** `2026-07-07 19:58:09 UTC`  
...
| **Raw URLs Discovered** | 0 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 0 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 0 | Graph Persistence Layer |
| **Reportable Endpoints** | 0 | Graph Memory Query |
...
| **Expansion Ratio** | **0x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (0) + \text{Parameters} (0) + \text{API Routes} (0) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 0x$$
...
|---|---|---|
| **Anonymous-only Routes** | 0 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (0)}{\max(1, \text{Anonymous-only Endpoints} (0))} = 1.0x$$
...
## 5. Discovery Verdict
🚨 **SHALLOW DISCOVERY WARNING (0x Expansion, 1.0x PER).** Only a minimal attack surface was mapped (1 endpoint). Downstream scanning coverage is extremely limited. Ensure that the target is not protected by aggressive WAF blocking, and that active crawling, session hijacking, and Wayback historical lookups are fully enabled.

...
| **Raw URLs Discovered** | 55 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 11 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 11 | Graph Persistence Layer |
| **Reportable Endpoints** | 11 | Graph Memory Query |
...
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (11) + \text{Parameters} (0) + \text{API Routes} (3) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 14x$$
...
|---|---|---|
| **Anonymous-only Routes** | 11 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (11)}{\max(1, \text{Anonymous-only Endpoints} (11))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **27.3%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (14x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

...
**Engagement ID:** `p0-e2e-20260725-152156`  
**Generated At:** `2026-07-25 15:26:04 UTC`  
...
| **Raw URLs Discovered** | 3 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 4 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 4 | Graph Persistence Layer |
| **Reportable Endpoints** | 4 | Graph Memory Query |
...
| **Expansion Ratio** | **4x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (4) + \text{Parameters} (0) + \text{API Routes} (0) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 4x$$
...
|---|---|---|
| **Anonymous-only Routes** | 4 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (4)}{\max(1, \text{Anonymous-only Endpoints} (4))} = 1.0x$$
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (4x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

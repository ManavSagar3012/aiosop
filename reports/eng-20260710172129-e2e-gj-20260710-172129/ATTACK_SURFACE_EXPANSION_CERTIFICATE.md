...
**Generated At:** `2026-07-11 02:28:42 UTC`  
**Discovery Level:** **MODERATE**  
**Expansion Ratio:** **561x**  
...
| **Discovered Subdomains** | 1 | Subfinder & Amass Passive/Active DNS |
| **Mapped Host IPs** | 0 | Port Scan / Infrastructure Mapping |
| **Raw URLs Discovered** | 1805 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 361 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 361 | Graph Persistence Layer |
| **Reportable Endpoints** | 361 | Graph Memory Query |
| **API / GraphQL Routes** | 0 | Route Pattern Matching |
| **JavaScript Bundles Found** | 18 | Static Resource Crawler |
| **Endpoints with Parameters** | 182 | Parameter Extraction Engine |
| **Expansion Ratio** | **561x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (361) + \text{Parameters} (182) + \text{API Routes} (0) + \text{JS Bundles} (18)}{\text{Input Targets} (1)} = 561x$$
...
|---|---|---|
| **Anonymous-only Routes** | 361 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (361)}{\max(1, \text{Anonymous-only Endpoints} (361))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **55.4%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).

### Discovery Verdict
{chr(45)} `ginandjuice.shop`

---

## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (561x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

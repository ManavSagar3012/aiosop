...
**Engagement ID:** `e2e-gj-20260725-185242`  
**Generated At:** `2026-07-25 19:07:11 UTC`  
**Discovery Level:** **MODERATE**  
**Expansion Ratio:** **509x**  
...
| **Raw URLs Discovered** | 595 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 314 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 314 | Graph Persistence Layer |
| **Reportable Endpoints** | 314 | Graph Memory Query |
| **API / GraphQL Routes** | 0 | Route Pattern Matching |
| **JavaScript Bundles Found** | 16 | Static Resource Crawler |
| **Endpoints with Parameters** | 179 | Parameter Extraction Engine |
| **Expansion Ratio** | **509x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (314) + \text{Parameters} (179) + \text{API Routes} (0) + \text{JS Bundles} (16)}{\text{Input Targets} (1)} = 509x$$
...
|---|---|---|
| **Anonymous-only Routes** | 314 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (314)}{\max(1, \text{Anonymous-only Endpoints} (314))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **62.1%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (509x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

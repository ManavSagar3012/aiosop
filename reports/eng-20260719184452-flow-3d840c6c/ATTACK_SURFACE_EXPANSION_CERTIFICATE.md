...
**Engagement ID:** `eng-20260719184452-flow-3d840c6c`  
**Generated At:** `2026-07-19 21:26:57 UTC`  
...
| **Raw URLs Discovered** | 131 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 6 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 6 | Graph Persistence Layer |
| **Reportable Endpoints** | 6 | Graph Memory Query |
| **API / GraphQL Routes** | 1 | Route Pattern Matching |
| **JavaScript Bundles Found** | 0 | Static Resource Crawler |
| **Endpoints with Parameters** | 1 | Parameter Extraction Engine |
| **Expansion Ratio** | **8x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (6) + \text{Parameters} (1) + \text{API Routes} (1) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 8x$$
...
|---|---|---|
| **Anonymous-only Routes** | 6 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (6)}{\max(1, \text{Anonymous-only Endpoints} (6))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **33.3%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (8x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

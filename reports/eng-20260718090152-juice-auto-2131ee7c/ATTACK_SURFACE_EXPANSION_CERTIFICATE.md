...
| **Raw URLs Discovered** | 134 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 112 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 112 | Graph Persistence Layer |
| **Reportable Endpoints** | 112 | Graph Memory Query |
| **API / GraphQL Routes** | 19 | Route Pattern Matching |
| **JavaScript Bundles Found** | 3 | Static Resource Crawler |
| **Endpoints with Parameters** | 80 | Parameter Extraction Engine |
| **Expansion Ratio** | **214x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (112) + \text{Parameters} (80) + \text{API Routes} (19) + \text{JS Bundles} (3)}{\text{Input Targets} (1)} = 214x$$
...
|---|---|---|
| **Anonymous-only Routes** | 112 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (112)}{\max(1, \text{Anonymous-only Endpoints} (112))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **91.1%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (214x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

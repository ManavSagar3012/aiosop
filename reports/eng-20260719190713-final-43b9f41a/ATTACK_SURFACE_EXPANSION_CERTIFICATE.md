...
**Engagement ID:** `eng-20260719190713-final-43b9f41a`  
**Generated At:** `2026-07-19 21:26:58 UTC`  
...
| **Deduplicated & Filtered Endpoints** | 8 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 8 | Graph Persistence Layer |
| **Reportable Endpoints** | 8 | Graph Memory Query |
| **API / GraphQL Routes** | 1 | Route Pattern Matching |
| **JavaScript Bundles Found** | 0 | Static Resource Crawler |
| **Endpoints with Parameters** | 2 | Parameter Extraction Engine |
| **Expansion Ratio** | **11x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (8) + \text{Parameters} (2) + \text{API Routes} (1) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 11x$$
...
|---|---|---|
| **Anonymous-only Routes** | 8 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (8)}{\max(1, \text{Anonymous-only Endpoints} (8))} = 1.0x$$
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (11x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

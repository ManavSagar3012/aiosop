...
**Engagement ID:** `eng-20260713092251-verify-syfe-145251`  
**Generated At:** `2026-07-13 09:35:30 UTC`  
**Discovery Level:** **MODERATE**  
**Expansion Ratio:** **297x**  
...
| **Raw URLs Discovered** | 353 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 170 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 170 | Graph Persistence Layer |
| **Reportable Endpoints** | 170 | Graph Memory Query |
| **API / GraphQL Routes** | 1 | Route Pattern Matching |
| **JavaScript Bundles Found** | 63 | Static Resource Crawler |
| **Endpoints with Parameters** | 63 | Parameter Extraction Engine |
| **Expansion Ratio** | **297x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (170) + \text{Parameters} (63) + \text{API Routes} (1) + \text{JS Bundles} (63)}{\text{Input Targets} (1)} = 297x$$
...
|---|---|---|
| **Anonymous-only Routes** | 170 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (170)}{\max(1, \text{Anonymous-only Endpoints} (170))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **74.7%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (297x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

...
**Generated At:** `2026-07-12 04:59:45 UTC`  
...
| **Raw URLs Discovered** | 175 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 38 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 38 | Graph Persistence Layer |
| **Reportable Endpoints** | 38 | Graph Memory Query |
| **API / GraphQL Routes** | 0 | Route Pattern Matching |
| **JavaScript Bundles Found** | 9 | Static Resource Crawler |
| **Endpoints with Parameters** | 18 | Parameter Extraction Engine |
| **Expansion Ratio** | **65x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (38) + \text{Parameters} (18) + \text{API Routes} (0) + \text{JS Bundles} (9)}{\text{Input Targets} (1)} = 65x$$
...
|---|---|---|
| **Anonymous-only Routes** | 38 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (38)}{\max(1, \text{Anonymous-only Endpoints} (38))} = 1.0x$$

---

## 4. Test Coverage Statement

The platform achieved an estimated **71.1%** coverage density on mapped endpoints. This means that high-fidelity attack vectors (such as API parameters, GraphQL schema resolvers, and client-side JavaScript bundles) were successfully extracted and fed into downstream vulnerability discovery tools (Burp Suite, Nuclei).
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (65x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

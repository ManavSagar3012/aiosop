...
| **Raw URLs Discovered** | 50 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 10 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 10 | Graph Persistence Layer |
| **Reportable Endpoints** | 10 | Graph Memory Query |
| **API / GraphQL Routes** | 9 | Route Pattern Matching |
| **JavaScript Bundles Found** | 0 | Static Resource Crawler |
| **Endpoints with Parameters** | 0 | Parameter Extraction Engine |
| **Expansion Ratio** | **19x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (10) + \text{Parameters} (0) + \text{API Routes} (9) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 19x$$
...
|---|---|---|
| **Anonymous-only Routes** | 10 | Accessible without session credentials |
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (19x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

...
**Generated At:** `2026-06-29 06:23:33 UTC`  
...
| **Raw URLs Discovered** | 10 | Active Crawler Link Harvester |
| **Deduplicated & Filtered Endpoints** | 2 | Noise Suppression Engine |
| **Persisted Endpoints in Neo4j** | 2 | Graph Persistence Layer |
| **Reportable Endpoints** | 2 | Graph Memory Query |
...
| **Expansion Ratio** | **2x** | Attestation Pipeline |

### 🧮 Expansion Ratio Formula
$$\text{Expansion Ratio} = \frac{\text{Persisted Endpoints} (2) + \text{Parameters} (0) + \text{API Routes} (0) + \text{JS Bundles} (0)}{\text{Input Targets} (1)} = 2x$$
...
|---|---|---|
| **Anonymous-only Routes** | 2 | Accessible without session credentials |
...
$$\text{PER} = \frac{\text{Total Endpoints} (2)}{\max(1, \text{Anonymous-only Endpoints} (2))} = 1.0x$$
...
## 5. Discovery Verdict
⚠️ **MODERATE DISCOVERY ACHIEVED (2x Expansion, 1.0x PER).** A basic attack surface was mapped. While sufficient for a standard assessment, deeper endpoint enumeration (such as custom wordlist directories or additional authenticated identities) is recommended for maximum coverage.

...
- ⚠️ **Observations (not falsehoods, but quality gaps):** (1) generated reports bloat to **7–8 MB** because per-finding evidence (raw HTTP bodies) is embedded untruncated — real data, but impractical for submission and heavy to render (injected via `dangerouslySetInnerHTML`). (2) The dashboard always follows the **latest** engagement — there is **no engagement selector**, so historical engagements' data can't be viewed in the UI. (3) `LATENCY 0MS` / `THROUGHPUT 0 EV/S` in the header appear to be placeholder/unwired telemetry (every other value is live). These are follow-ups, not blockers.

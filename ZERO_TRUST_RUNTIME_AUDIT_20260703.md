...
- ⚠️ **Observations (not falsehoods, but quality gaps) — remediation status:** (1) ✅ **FIXED** — generated reports bloated to **7–8 MB** (untruncated per-finding evidence); now capped at `report_evidence_max_chars` (4000) → regenerated report **7.2 MB → 192 KB** with a vault reference + full-artifact hash retained (AIOSOP-REPORT-TRUNC-001). (2) `no engagement selector` — the dashboard follows the latest engagement only; historical data can't be browsed in the UI (open UI follow-up). (3) ✅ **FIXED** — `LATENCY`/`THROUGHPUT` header telemetry read 0 because the WebSocket handler was request-response only and never *pushed* events; see §4.11.
...
### 4.11 Live WebSocket telemetry/updates never fired (FIXED, AIOSOP-WS-PUSH-001)
- 🔴 The dashboard's WS client subscribes to pushed events (`heartbeat`→latency, `phase_transition`, `finding_update`, `graph_update`), but the backend `@app.websocket("/ws/engagements/{id}")` handler was **request-response only** — it replied to client `ping`/`status`/`halt` and never *pushed* anything. So `LATENCY`/`THROUGHPUT` sat at 0 and the UI only refreshed on navigation (no real-time updates).
- **Fix:** the handler now runs a concurrent push loop that emits a `heartbeat` (carrying a real Redis-PING round-trip latency) every 2s plus a `phase_transition` event whenever the engagement's phase changes; the loop is cancelled cleanly on disconnect. **Verified:** a raw WS client received `heartbeat` + `phase_transition` (phase=exploitation); on the live dashboard the throughput counter now ticks `0↔1 EV/S` (was stuck 0) and the browser logs `[Network] WS Connected`. Latency displays ~0 because local Redis PING is genuinely sub-millisecond (honest, not a stub).
- ↪️ **Remaining follow-up (open):** the full orchestrator event bus (`finding_update`, `agent_observation`) is not yet forwarded over the WS, so throughput reflects heartbeat cadence rather than real swarm event volume, and new findings still require a page navigation to appear. Wiring the event bus to the socket is the next enhancement (a feature, not a correctness fix).

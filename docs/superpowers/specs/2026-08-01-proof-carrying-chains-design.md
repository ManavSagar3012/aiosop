...
- Phase gating has a **dead/policy-shadow bug**: `core/config.py:592-632` defines `PHASE_POLICY` with `requires_manual_approval=True` for EXPLOITATION — but nothing imports it, and `Orchestrator.PHASE_POLICY` (`orchestrator.py:61-86`) uses different key names (`manual_approval`/`auto_next`) with every phase set to `False`. Only per-task `approval_id` gates exploitation today.
...
- Delete the dead `core/config.py:592-632` `PHASE_POLICY` (nothing imports it; it conflicts with the orchestrator's live table). Grep-verified 2026-08-01: no production or test import of the config shadow.
...
2. **Lab verification against Juice Shop** (`benchmarks/juiceshop/`): with the engine's known vuln classes validated end-to-end — payload → sandbox execution → oracle verdict → `ExploitReceipt` on disk and Postgres → `verify_chain` True. Blind-XSS/SQLi/SSTI verified against a purpose-built sink fixture in `tests/qualification/conftest.py` (an ephemeral `http.server` route that performs a callback on trigger, mirroring `OASTAdapter`), plus a documented manual run against the external Juice Shop convention.
...
- The executor's abort path sets `chain_failed` via the existing ledger legality rules; a `partial` chain never transitions to `successful_chain`.

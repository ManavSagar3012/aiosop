...
import { HelpCircle, AlertOctagon, Lightbulb, Search, ArrowRight } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center bg-surface-container p-4 border border-outline-variant">
         <div className="flex flex-col">
            <span className="font-label-caps text-[9px] text-on-surface-variant mb-1 uppercase">Reasoning Mode</span>
         ...
         <div className="flex gap-4">
            <div className="bg-black/40 px-4 py-2 border border-outline-variant text-center">
               ...
               <div className="font-label-caps text-on-surface-variant text-[9px]">TOTAL UNKNOWNS</div>
            </div>
            <div className="bg-black/40 px-4 py-2 border border-outline-variant text-center">
               <div className="font-code-sm text-error text-[14px] font-bold">{blockedPathsCount}</div>
               <div className="font-label-caps text-on-surface-variant text-[9px]">BLOCKED PATHS</div>
            </div>
         </div>
      </div>
              ...
              {uncertainties.length > 0 ? uncertainties.map(unc => (
                <div key={unc.id} className="bg-surface-container-high border border-outline-variant p-5">
                   <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="font-headline-md text-primary">{unc.target}</div>
                   </div>

                   <div className="grid grid-cols-2 gap-8">
                      <div>
                         <div className="font-label-caps text-[9px] text-on-surface-variant mb-3 uppercase tracking-widest">Formal Unknowns</div>
                      ...
                      <div>
                         <div className="font-label-caps text-[9px] text-on-surface-variant mb-3 uppercase tracking-widest">Blocked Discovery Paths</div>
                         <div className="space-y-2">
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-20 italic font-code-sm py-20">
                     <Search size={48} className="mb-4" />
                     No knowledge boundaries identified yet.
                  </div>
              )}
           </div>
        </Card>
           ...
           <div className="h-full font-code-sm text-[11px] text-on-surface-variant leading-relaxed space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                 <div className="border border-outline-variant p-3">
                    <div className="font-label-caps text-[9px] text-primary-fixed mb-2">MOST UNCERTAIN STACK</div>
                    <div className="text-on-surface">Cloudflare Turnstile + Custom WebGL</div>
                 </div>
                 <div className="border border-outline-variant p-3">
                    <div className="font-label-caps text-[9px] text-primary-fixed mb-2">HIGHEST DATA GAP</div>

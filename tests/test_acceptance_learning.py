...
from ai_osop.core.models import Vulnerability, OutcomeStatus, OutcomeRecord
...
async def test_acceptance_yield_scoring():
    ...
    outcomes = [
        OutcomeRecord(finding_id="v1", finding_type="xss", status=OutcomeStatus.DUPLICATE, severity="medium", agent_id_responsible="triage-agent", engagement_id="eng-1"),
        OutcomeRecord(finding_id="v2", finding_type="xss", status=OutcomeStatus.INFORMATIVE, severity="medium", agent_id_responsible="triage-agent", engagement_id="eng-1"),
        OutcomeRecord(finding_id="v3", finding_type="idor", status=OutcomeStatus.ACCEPTED, severity="high", agent_id_responsible="triage-agent", engagement_id="eng-1")
    ]
    ...
    def calculate_ays(findings, outcomes):
        ...
        by_id = {"anonymous": {"total": 0, "accepted": 0}, "user_a": {"total": 0, "accepted": 0}, "admin": {"total": 0, "accepted": 0}}
        ...
        return {
            tool: (stats["accepted"] / stats["total"]) for tool, stats in by_tool.items()
        }, {

...
def _res(value, engagement_id="t"):
    return Resource(id=f"r:{value}", type="basket", value=str(value),
                    owner_identity_id="victim@x", metadata={}, engagement_id=engagement_id)
...
def test_true_cross_account_read_still_detected():
    ...
    attacker = {"status_code": 200, "body": {"id": 42, "email": "victim@x", "items": ["a"]},
                "user_label": "attacker"}
    anon = {"status_code": 401, "body": {"error": "unauthorized"}}
    finding = _run(engine.compare(
        identity_a_evidence=owner,

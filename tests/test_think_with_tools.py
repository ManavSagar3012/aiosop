...
class _ToolAgent(BaseAgent):
    ...
    def __init__(self, llm):
        self.ctx = SimpleNamespace(
            agent_id="a1", agent_type=AgentType.RECON, llm_client=llm,
            scope=None, rate_limiter=None,
        )

    def agent_type(self):  # type: ignore[override]
        return AgentType.RECON

    async def _setup_resources(self): return None
    async def _cleanup_resources(self): return None
    async def _execute(self, task): return {}  # noqa: ANN001

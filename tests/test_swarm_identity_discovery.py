...
from ai_osop.core.models import Task, Endpoint
...
async def test_swarm_identity_crawling():
    ...
    with MagicMock() as mock_store:
        ...
        # Setup Task
        task = Task(id="t1", type="full_recon", agent_type=AgentType.RECON, engagement_id="eng-1", payload={"domain": "target.com"})
        ...
        def get_mock_response(url, **kwargs):
            ...
            if "token-user-a" in auth:
                ...
                resp.text = AsyncMock(return_value="<html><body><a href='/user_a_dashboard'>link</a></body></html>")
            elif "token-admin" in auth:
                resp.url = "https://target.com/admin_only"
                resp.text = AsyncMock(return_value="<html><body><a href='/admin_dashboard'>link</a></body></html>")
            else:
                resp.url = "https://target.com/anonymous_only"
                resp.text = AsyncMock(return_value="<html><body><a href='/public_dashboard'>link</a></body></html>")

...
def test_coverage_floor_is_at_least_70():
    ...
    cfg = (Path(__file__).parent.parent / "pyproject.toml").read_text(encoding="utf-8", errors="ignore")
...
async def test_mock_llm_logs_once_per_process():
    ...
    with patch.object(llm_client, "settings") as fake, patch.object(llm_client, "llm_logger") as fake_logger:
        fake.mock_llm = True

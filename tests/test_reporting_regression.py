...
async def test_reporting_stats_keys():
    ...
    ctx.graph_memory.get_graph_stats = AsyncMock(
        return_value={
            "assets": 12,
            "endpoints": 4,
            "total_nodes": 16,
...
async def test_reporting_graph_render():
    ...
    ctx.graph_memory.get_graph_stats = AsyncMock(
        return_value={"assets": 2, "endpoints": 1, "total_nodes": 3}
    )
    ctx.llm_client.complete = AsyncMock(return_value="Mocked risk narrative.")
    # Mock the new encapsulated GraphMemory methods
    ctx.graph_memory.get_vulnerabilities_by_engagement = AsyncMock(return_value=[])
    ctx.graph_memory.get_all_nodes_for_engagement = AsyncMock(
        return_value=[
            {"id": "asset-1", "labels": ["Asset", "Domain"]},
            {"id": "endpoint-1", "labels": ["Endpoint"]},
        ]
    )
    ctx.graph_memory.get_all_edges_for_engagement = AsyncMock(
        return_value=[{"source": "asset-1", "target": "endpoint-1", "type": "HAS_ENDPOINT"}]
    )
...
async def test_finding_certification_engine():
    ...
    graph_memory_mock.get_graph_stats = AsyncMock(
        return_value={"assets": 10, "endpoints": 5, "total_nodes": 15}
    )
    graph_memory_mock.run_read_query = AsyncMock(
        return_value=[
            {
                "v": {
                    "id": "vuln-high-1",

...
from ai_osop.core.models import Task, Asset, Endpoint, ScopeDefinition, SessionState
...
@pytest.mark.skipif(not is_db_available(), reason="Database services (Postgres, Neo4j, Redis) are not running.")
@pytest.mark.asyncio
async def test_e2e_recon_to_reporting_pipeline():
    ...
    try:
        ...
        async with sm._async_session() as session:
            ...
            await session.execute(text("DELETE FROM audit_logs WHERE engagement_id = :eid"), {"eid": engagement_id})
            await session.execute(text("DELETE FROM tasks WHERE engagement_id = :eid"), {"eid": engagement_id})
            await session.execute(text("DELETE FROM session_states WHERE session_id = :eid"), {"eid": engagement_id})

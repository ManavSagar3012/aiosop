...
def test_state_machine():
    ...
    print(f"Transition to RECONNAISSANCE: {success}, New state: {state.phase}, Version: {state.version}")
    assert success
    ...
    print(f"Transition to INITIALIZED: {success}, New state: {state.phase}, Version: {state.version}")
    assert not success
    assert state.phase == EngagementPhase.RECONNAISSANCE
    assert state.version == 1

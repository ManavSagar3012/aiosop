...
def _fake_session(eng_id: str, phase: str):
    ...
    return SimpleNamespace(session_id=eng_id, canonical_engagement_id=eng_id, phase=phase)
...
async def _run():
    ...
    t_halt = Task(
        ...
        engagement_id="eng-halted",
        status="running",
    )
    t_active = Task(
        type="register", agent_type=AgentType.WORKFLOW, engagement_id="eng-active", status="pending"
    )

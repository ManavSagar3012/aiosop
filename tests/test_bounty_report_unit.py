...
def _vuln(**over):
    base = dict(
        vuln_type=VulnClass.SSRF, severity=Severity.HIGH, cwe="CWE-918",
        title="Blind SSRF via imageUrl",
        description="Server fetched attacker URL.",
        evidence=[{"type": "ssrf_callback", "url": "http://t/profile/image/url",
                   "injection": "imageUrl", "interaction": {"method": "GET", "path": "/abc"}}],
        tool_source="oast_ssrf", confidence=0.97, validated=True, engagement_id="e1",
    )
    base.update(over)
...
def test_report_has_core_sections():
    ...
    for section in ("# ", "## Summary", "## Steps to Reproduce",
                    "## Impact", "## Evidence", "## Remediation", "Severity"):
...
def test_report_for_secret_uses_redacted_value():
    v = _vuln(vuln_type=VulnClass.EXPOSED_SECRET, cwe="CWE-798",
              title="Live github credential",
              evidence=[{"type": "live_credential", "provider": "github",
                         "secret_redacted": "ghp_...00 (len 40)", "verify_status": 200}])

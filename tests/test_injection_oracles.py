...
from ai_osop.core.injection_oracles import plant_blind_xxe, _blind_xxe_bodies
...
class _FakeOAST:
    ...
    def fire(self, token, source_ip="203.0.113.7"):
        ...
        self._interactions.append({
            "seq": self._seq, "token": token, "interaction_id": f"i{self._seq}",
            "kind": "http", "source_ip": source_ip, "method": "GET",
            "path": f"/{token}", "context": self._tokens.get(token, {}),
        })

    async def drain(self, since=0, engagement_id=None):
        out = [i for i in self._interactions if i["seq"] > since
...
async def test_plant_blind_xxe_confirms_via_oob_callback():
    ...
    async with _client(app, "http://vuln.test") as c:
        planted = await plant_blind_xxe(
            c, "http://vuln.test/stock", oast_registry=reg,
            engagement_id="eng1", sample_xml=_STOCK_SAMPLE,
        )
    assert planted >= 1
...
async def test_plant_blind_xxe_no_finding_without_callback():
    ...
    async with _client(app, "http://safe.test") as c:
        planted = await plant_blind_xxe(
            c, "http://safe.test/stock", oast_registry=reg,
            engagement_id="eng2", sample_xml=_STOCK_SAMPLE,
        )
...
async def test_plant_blind_xxe_noop_without_registry():
    async with _client(_blind_app_factory(_FakeOAST(), vulnerable=True), "http://vuln.test") as c:
        planted = await plant_blind_xxe(
            c, "http://vuln.test/stock", oast_registry=None, engagement_id="e",
        )
    assert planted == 0

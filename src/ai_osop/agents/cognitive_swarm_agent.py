...
class CognitiveSwarmAgent(ABC):
    ...
    async def publish(
        ...
        logger.debug(
            f"[{self.agent_id}] Published to {topic}: {payload.get('summary', 'No summary')}"
        )
...
class VulnerabilityCorrelationAgent(CognitiveSwarmAgent):
    ...
    def __init__(self, engagement_id: str = "default"):
        super().__init__(
            agent_id="vuln_correlator_01", agent_type="vulnerability", engagement_id=engagement_id
        )
        ...
        if len(self.knowledge_base) >= 3:
            ...
            if chain:
                await self.publish(
                    ...
                    payload={"chain": chain, "severity": "critical", "steps": len(chain)},
...
async def run_swarm_demo(engagement_id: str = "demo-eng"):
    ...
    try:
        await asyncio.gather(vuln_agent.run(), chain_agent.run(), simulate_recon())

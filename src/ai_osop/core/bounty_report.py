...
_CVSS = {  # rough representative vectors for the report header
    "critical": "9.8 (Critical)", "high": "8.1 (High)", "medium": "5.4 (Medium)",
    "low": "3.1 (Low)", "info": "0.0 (Informational)",
}
...
def _primary_endpoint(ev: Dict[str, Any]) -> str:
    return (ev.get("url") or ev.get("verify_url") or ev.get("store_url")
            or ev.get("host") or ev.get("render_url") or "")
...
def finding_signature(vuln: Vulnerability) -> str:
    ...
    inj = str(ev.get("injection") or ev.get("parameter") or ev.get("store_field")
              or ev.get("provider") or ev.get("service") or "")
...
def _repro_steps(vuln: Vulnerability) -> List[str]:
    ...
    if vt == "ssrf":
        ...
        return [f"Send a request to `{ep}` with the `{ev.get('injection')}` field set to an "
    ...
    if vt == "sqli":
        ...
        return [f"Target parameter `{ev.get('parameter')}` at `{ep}`.",
                f"Inject the payload: `{pl}`.",
                f"sqlmap confirms injection (DBMS: {ev.get('dbms','unknown')})."]
    if vt == "xss":
        return [f"Submit the payload into `{ev.get('store_field') or ev.get('parameter') or 'the field'}` "
    ...
    if vt == "jwt_abuse" or vt == "broken_access_control":
        return [f"Take a valid JWT and apply the `{ev.get('technique','forgery')}` technique.",
    ...
    if vt == "mass_assignment":
        return [f"Send the create/update request to `{ep}` adding the privileged field(s) "
                f"{ev.get('accepted_fields')}.",
                "Confirm the privileged value is persisted/reflected."]
    if vt == "subdomain_takeover":
        return [f"Resolve `{ev.get('host')}` — it points at an unclaimed {ev.get('service')} resource "
                f"(signature: '{ev.get('signature')}').",
                f"Claim the resource on {ev.get('service')} to serve content under the domain."]
    if vt == "exposed_secret":
        return [f"Extract the {ev.get('provider')} secret ({ev.get('secret_redacted')}).",
    ...
    if vt == "race_condition":
        return [f"Send {ev.get('concurrency')} synchronized single-packet requests to `{ep}`.",
...
def render_bounty_report(vuln: Vulnerability, program: str = "") -> str:
    ...
    header += (f"**Severity:** {vuln.severity if not hasattr(vuln.severity,'value') else vuln.severity.value} "

...
async def get_hypotheses(
    ...
    """Return hypotheses generated by the HypothesisEngine.

    Each hypothesis is a testable security inference: 'this endpoint may
    have an authorization bypass because it accepts auth tokens and has
    admin in the path.' The reasoning loop selects the highest-confidence
    hypothesis and dispatches it as a task.
    """
    await assert_engagement_access(operator, session_id)
    ...
    if refresh:
        ...
        result = [h.model_dump() if hasattr(h, "model_dump") else dict(h) for h in hypotheses]
    else:
        stored = await orch.graph_memory.get_hypotheses_by_engagement(session_id)
        result = [dict(item) for item in stored[:limit]]
        if not result:
            hypotheses = await engine.generate_hypotheses(session_id, focus=focus, limit=limit)
            result = [h.model_dump() if hasattr(h, "model_dump") else dict(h) for h in hypotheses]
    return {"session_id": session_id, "count": len(result), "hypotheses": result}

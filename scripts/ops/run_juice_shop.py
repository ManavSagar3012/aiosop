...
async def main():
    print("=" * 60)
    print("AI-OSOP E2E VERIFICATION RUNNER - TARGET: DEMO.OWASP-JUICE.SHOP")
    print("=" * 60)
    ...
    engagement_id = f"e2e-juice-{stamp}"
    print(f"Creating engagement: {engagement_id}")

    # Target a LOCAL Juice Shop container (docker run -p 3000:3000 bkimminich/juice-shop).
    # The public Heroku demo is a free dyno that 503s under load — useless for
    # reproducible evidence. domains carries the host:port so recon seeds
    # http://127.0.0.1:3000/; ips carries the bare IP so ScopeEnforcer.host_in_scope
    # (which routes 127.0.0.1 through its IP branch) accepts it.
    payload = {
        "engagement_id": engagement_id,
        "domains": ["127.0.0.1:3000"],
        "ips": ["127.0.0.1"],
        "allowed_techniques": ["passive", "active"],
        "authorization_ref": "E2E-JUICESHOP-VERIFY",
        "roe": {"note": "E2E verification run against local OWASP Juice Shop :3000"},
    }
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        # CONFIRMED-LIVE manifest (verified by hand 2026-07-26 against
        # demo.owasp-juice.shop before this run — see session notes):
        #   - GET /rest/products/search?q=  UNION SQLi: q=')) UNION SELECT 1-- -> HTTP 500;
        #     q='))-- -> 200 w/ full product dump (comment breaks the WHERE filter).
        #   - POST /rest/user/login  email SQLi: {"email":"' OR 1=1--"} -> 200 + admin JWT
        #     (auth bypass; wrong creds -> 401).
        expected_manifest = [
            {
                "vuln_class": "sqli",
                "path": "/rest/products/search",
                "parameter": "q",
                "description": "UNION/boolean SQL injection in product search (GET q param)",
            },
            {
                "vuln_class": "sqli",
                "path": "/rest/user/login",
                "parameter": "email",
                "description": "SQL injection auth bypass in login (email field) -> admin JWT",
            },
        ]
